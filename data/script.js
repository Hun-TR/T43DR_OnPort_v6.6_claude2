    ...
    secureFetch('/api/ntp')
        ...
        .then(ntp => {
            if (ntp) {
                ...
                // *** YENİ: Senkronizasyon durumu kontrolü ***
                // Backend'den sync durumunu al (eğer gönderiliyorsa)
                if (ntp.syncStatus !== undefined) {
                    const syncStatusEl = document.getElementById('syncStatus');
                    if (syncStatusEl) {
                        if (ntp.syncStatus === 'synced') {
                            syncStatusEl.textContent = '✅ Senkron';
                            syncStatusEl.style.color = 'var(--success)';
                        } else if (ntp.syncStatus === 'updated') {
                            syncStatusEl.textContent = '⚠️ Güncellendi';
                            syncStatusEl.style.color = 'var(--warning)';
                        } else {
                            syncStatusEl.textContent = '❓ Bilinmiyor';
...
function initFaultPage() {
    ...
    // ULTRA HIZLI tek arıza alma
    async function getSingleFaultUltraFast(faultNo) {
        try {
            const command = faultNo.toString().padStart(5, '0') + 'v';
            const formData = new URLSearchParams();
            formData.append('command', command);
            
            // Timeout'u 300ms'e düşür
            ...
            return null; // Hataları sessizce geç
        }
    }
    
    // ULTRA HIZLI TÜM ARIZALARI AL
    async function fetchAllFaultsUltraFast() {
        if (isLoading) return;
        isLoading = true;
        ...
        try {
            ...
            showMessage(`✅ ${totalCount} arıza bulundu, hızlı çekiliyor...`, 'info');
            updateProgress(0, totalCount);
            
            // PARALEL İSTEK SİSTEMİ - OPTİMİZE EDİLMİŞ
            const MAX_PARALLEL = 5; // 10 yerine 5'e düşür (dsPIC zorlanmasın)
            let completed = 0;
            let successCount = 0;
            let failCount = 0;
            ...
            async function processBatch(batchStart, batchEnd) {
                ...
                for (let i = batchStart; i >= batchEnd && i >= 1; i--) {
                    promises.push(
                        getSingleFaultUltraFast(i).then(fault => {
                            completed++;
                            ...
                            if (fault) {
                                ...
                                renderBuffer.push(fault);
                                
                                // Her 25 kayıtta bir güncelle
                                if (completed % 25 === 0 || completed === totalCount) {
                                    updateProgress(completed, totalCount);
                                    updateElement('progressText', 
                                        `Çekiliyor: ${completed}/${totalCount} (${Math.round(completed/totalCount*100)}%)`);
                                }
                                
                                // Her 50 kayıtta bir render
                                if (renderBuffer.length >= 50) {
                                    batchRenderTable();
                                }
                            } else {
                                failCount++;
                                ...
                                if (consecutiveErrors > 10) {
                                    console.warn('⚠️ Çok fazla ardışık hata, yavaşlatılıyor...');
                                    return 'slow_down';
                                }
                            }
                    ...
                    // Her MAX_PARALLEL istekte bekle
                ...
                // Kalan istekleri tamamla
                if (promises.length > 0) {
                    await Promise.all(promises);
                }
            }
            
            // BATCH'LER HALİNDE İŞLE
            const BATCH_SIZE = 100;
            for (let i = totalCount; i >= 1; i -= BATCH_SIZE) {
                const batchStart = i;
                const batchEnd = Math.max(1, i - BATCH_SIZE + 1);
                ...
                if (consecutiveErrors > 20) {
                    throw new Error('Çok fazla ardışık hata, işlem durduruluyor');
                }
            }
            
            // Son kalan kayıtları render et
            if (renderBuffer.length > 0) {
                batchRenderTable();
            }
            
            updateProgress(totalCount, totalCount);
            updateElement('progressText', `✅ Tamamlandı!`);
            updateElement('lastQuery', new Date().toLocaleTimeString());
            
            const elapsedTime = Math.round((Date.now() - startTime) / 1000);
            const avgSpeed = successCount > 0 ? Math.round(successCount / elapsedTime * 60) : 0;
            
            showMessage(
                `✅ ${successCount}/${totalCount} kayıt ${elapsedTime} saniyede alındı` +
            ...
            console.error('Ultra hızlı arıza çekme hatası:', error);
    ...
    fetchAllFaultsBtn.addEventListener('click', fetchAllFaultsUltraFast);
